...
import com.eodya.api.users.repository.*;
...
public class PlaceService {
   ...
   public void createPlaceAndReview(Long userId, PlaceCreateRequest request) {
       ...
       Tag tag = tagRepository.findByName(request.getTag())
               .orElseGet(() ->
               {
                PlaceStatus newPlaceTag = request.toTagEntity(request.getTag());
       ...
       if (findPlace.isPresent()) {
           Optional<PlaceStatus> placeTag = placeTagRepository.findByPlaceAndTag(findPlace.get(), tag);
           ...
           Optional<AddressDepth2> findDepth2 = addressDepth1.getDepth2().stream() 
